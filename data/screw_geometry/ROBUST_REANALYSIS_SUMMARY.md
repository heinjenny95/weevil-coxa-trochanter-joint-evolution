...
- The current multivariate shape comparison uses 15 shape-only proxy tips and
  records optimizer convergence separately from Hessian reliability. For
  PC1--PC5, the OU optimizer converges but the Hessian solution is unreliable;
  for PC1--PC4, the OU fit fails. BM and EB are reliable in both comparisons,
  with BM ranked first among reliable fits. Raw OU AIC support is retained only
  for transparency and is not used for biological inference.
- Continuous ancestral-state reconstructions are conditional proxy-tree
  summaries. Raw-scale node estimates, variances and 95% intervals are exported
  for PC1--PC5 and centroid size on 15 shape-only tips and for absolute winding
  angle and endpoint-based axial span on 14 geometry tips. Figure colours use
  within-trait standardized values, but each map retains the same trait-specific
  tip set as its numerical reconstruction. The reconstructions are explicitly
  conditional on Brownian motion (BM), which is recorded in the numerical
  tables and figure legend as a reconstruction assumption rather than a
  model-selection result. This distinction matters for axial span, for which OU
  is top-ranked in the exploratory primary-tree univariate comparison. These
  outputs do not justify genus-, family- or clade-level ancestral-state claims.
