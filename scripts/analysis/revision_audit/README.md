...
`refresh_shape_geometry_tables.R` rebuilds the concise primary and strict
shape--geometry summaries from the canonical regression tables and validates
the expected sample sizes (63 and 53). `sync_supplementary_source_data.py`
then synchronizes Supplementary Tables 13--39 with the robust point-estimate
outputs, labels combined files by quality set, removes private workstation
paths, normalizes all released tables to comma-delimited CSV with decimal
