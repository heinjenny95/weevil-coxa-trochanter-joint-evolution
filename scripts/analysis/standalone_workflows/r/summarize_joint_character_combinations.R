...
# - Observed vs missing combos
# - Family split: counts + within-family proportions + presence/absence
# - Optional: Subfamily + Group
# - Exports CSV + PDF plots
#
...
# plot filter: ignore families with tiny n to avoid unreadable plots
MIN_N_FAMILY_FOR_PLOTS <- 3

# -------------------- PACKAGES --------------------
pkgs <- c("readxl", "dplyr", "tidyr", "stringr", "ggplot2", "forcats", "readr")
...
norm_tristate <- function(x) {
  ...
  dplyr::case_when(
    ...
    TRUE ~ y  # unexpected -> keep original for warning
  )
}
...
# Normalize column names (handles NBSP etc.)
...
if (anyDuplicated(df$specimen_id) > 0) {
  ...
  write.csv(data.frame(specimen_id = dup_ids),
...
# Keep only complete rows (traits + Family)
df_complete <- df %>%
  dplyr::filter(
    !is.na(Family), Family != "",
    !is.na(Schraube),
...
df_complete <- df_complete %>%
  dplyr::mutate(
    ...
    combo = interaction(Schraube, `Coxal wall hole`, `Coxal Socket`, `Windung Coxa`,
                        sep = "_", drop = FALSE)
  )

# All theoretical combos (24)
...
write.csv(obs_overall %>% dplyr::filter(is_missing),
...
presence_by_family <- obs_by_family %>%
  ...
  dplyr::select(Family, combo, present, Schraube, `Coxal wall hole`, `Coxal Socket`, `Windung Coxa`, n, prop_in_family, n_family_total)

write.csv(presence_by_family, file.path(out_dir, "combos_by_family_presence.csv"), row.names = FALSE)

# Family summary: top combo per family
top_combo_by_family <- obs_by_family %>%
  dplyr::filter(n > 0) %>%
  dplyr::group_by(Family) %>%
  dplyr::arrange(dplyr::desc(n), combo) %>%
  dplyr::slice(1) %>%
  dplyr::ungroup() %>%
  dplyr::select(Family, n_family_total, combo, Schraube, `Coxal wall hole`, `Coxal Socket`, `Windung Coxa`, n, prop_in_family)

write.csv(top_combo_by_family, file.path(out_dir, "top_combo_by_family.csv"), row.names = FALSE)
...
# Data for family plots (filtered)
fam_plot <- obs_by_family %>% dplyr::filter(n_family_total >= MIN_N_FAMILY_FOR_PLOTS)

# 2) Family x combo heatmap (within-family proportions)
p2 <- fam_plot %>%
  dplyr::mutate(
    combo = forcats::fct_reorder(combo, n, .fun = sum, .desc = TRUE),
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(
    title = paste0("Joint-trait combinations by family (within-family proportions; n>=", MIN_N_FAMILY_FOR_PLOTS, ")"),
    x = "Combination",
    y = "Family",
    fill = "Proportion"
  )

ggsave(file.path(out_dir, "PLOT_family_combo_heatmap_props.pdf"),
       p2, width = 14, height = max(6, 0.25 * length(unique(fam_plot$Family))))

# 3) Presence/absence heatmap (family  combo)
p3 <- fam_plot %>%
  dplyr::mutate(
    present = ifelse(n > 0, "present", "absent"),
    combo = forcats::fct_reorder(combo, n, .fun = sum, .desc = TRUE),
    Family = forcats::fct_reorder(Family, n_family_total, .desc = TRUE)
  ) %>%
  ggplot(aes(x = combo, y = Family, fill = present)) +
  geom_tile() +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(
    title = paste0("Presence/absence of combinations by family (n>=", MIN_N_FAMILY_FOR_PLOTS, ")"),
    x = "Combination",
    y = "Family",
    fill = ""
  )

ggsave(file.path(out_dir, "PLOT_family_combo_presence_absence.pdf"),
       p3, width = 14, height = max(6, 0.25 * length(unique(fam_plot$Family))))

# 4) Mechanically clean view: by Schraube facet (counts)
p4 <- fam_plot %>%
  dplyr::mutate(
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(
    title = paste0("Family distribution split by Schraube state (n>=", MIN_N_FAMILY_FOR_PLOTS, ")"),
...
ggsave(file.path(out_dir, "PLOT_family_heatmap_by_schraube_counts.pdf"),
       p4, width = 14, height = max(6, 0.25 * length(unique(fam_plot$Family))))
...
summary_lines <- c(
  ...
  paste0("Families total: ", length(unique(df_complete$Family))),
  paste0("Families plotted (n >= ", MIN_N_FAMILY_FOR_PLOTS, "): ", length(unique(fam_plot$Family))),
  "",
  "Top 10 observed combos (combo : n_observed):",
  paste0(capture.output(print(
    head(obs_overall %>% dplyr::filter(n_observed > 0) %>% dplyr::select(combo, n_observed), 10),
    row.names = FALSE
  )), collapse = "\n"),
  "",
  "Top combo per family written to: top_combo_by_family.csv",
  "Outputs written to:",
  out_dir
)
