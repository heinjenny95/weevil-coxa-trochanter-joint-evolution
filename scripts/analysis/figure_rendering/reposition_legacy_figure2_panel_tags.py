...
def reposition_png(source: Path, destination: Path, layout: dict[str, object]) -> None:
...
def reposition_pdf(source: Path, destination: Path, layout: dict[str, object]) -> None:
    ...
    overlay.setFont("Arial-Bold", layout["font_points"])
    baseline_offset = 35
    for label, (x, y) in zip(layout["labels"], layout["new_positions"], strict=True):
...
def main() -> None:
    ...
    reposition_png(args.input_png, args.output_png, layout)
    reposition_pdf(args.input_pdf, args.output_pdf, layout)
